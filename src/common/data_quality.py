...
def required(df: DataFrame, columns: list[str]) -> DataFrame:
    if not columns:
        return df.withColumn("_dq_required_failed", F.lit(False))
...
def non_negative(df: DataFrame, columns: list[str]) -> DataFrame:
    if not columns:
        return df.withColumn("_dq_negative_failed", F.lit(False))
...
def status(df: DataFrame) -> DataFrame:
    bad = (
        F.coalesce(F.col("_dq_required_failed"), F.lit(False))
        | F.coalesce(F.col("_dq_negative_failed"), F.lit(False))
    )

    return df.withColumn(
        "dq_status",
        F.when(bad, "QUARANTINE").otherwise("VALID")
    )